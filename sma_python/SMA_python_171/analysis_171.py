...
def process_single_image_171(image_path, params, config):
    """
    ...
    if config.get('use_tubeness_filter', False):
        # Flujo de trabajo alternativo usando filtro de Tubeness (Sato)
        sigma_val = int(params.get("Tsigma", 10))
        if sigma_val == 0: sigma_val = 1 # Sato filter requires sigma > 0

        # El filtro Sato devuelve valores más bajos para estructuras más tubulares.
        # Lo invertimos para que las aponeurosis sean brillantes.
        sato_filtered = sato(img_clahe, sigmas=range(sigma_val, sigma_val + 1), black_ridges=False)

        # Normalizar para que se pueda visualizar y usar en Canny
        img_filtered = cv2.normalize(sato_filtered, None, 0, 255, cv2.NORM_MINMAX, dtype=cv2.CV_8U)

    else:
        # Flujo de trabajo por defecto (basado en FFT)
        img_filtered = fft_filter_and_threshold(img_clahe, percentile=99.9)

    # Detección de bordes con Canny
    edges = cv2.Canny(image=img_filtered, threshold1=50, threshold2=150)
...
_DEFAULT_GENERAL_CONFIG = {
    ...
    "use_tubeness_filter": False # Usar filtro Tubeness en lugar de FFT para aponeurosis
}
...
def SMA(input_image_path, output_path, analysis_params=None, general_config=None):
    ...
    Notas:
        Al portar la lógica del macro original de ImageJ a Python, se tomaron
        algunas decisiones para simplificar y utilizar las fortalezas de las
        bibliotecas de Python (OpenCV, Scikit-image).

        - `Tsigma`: Este parámetro controlaba el filtro "Tubeness" en ImageJ.
          En esta implementación, el flujo de trabajo por defecto utiliza un
          filtrado por FFT seguido de Canny, que no requiere `Tsigma`. Se ha
          añadido una opción en `general_config` (`use_tubeness_filter`) para
          permitir el uso de un filtro de Tubeness (Sato) como alternativa,
          que sí utiliza `Tsigma`.
        - `extrapolate_from`: En el macro, permitía usar el 50% o el 100% de la
          aponeurosis para la regresión lineal. La implementación actual en
          Python con `numpy.polyfit` siempre usa todos los puntos detectados,
          lo que equivale a la opción del 100%. La opción del 50% no se implementó
          para mantener el código más directo.
        - Otros parámetros como `ROIn`, `ROIwidth`, `ROIheight`, etc., no se
          utilizan porque la detección de fascículos se realiza sobre una única
          ROI grande definida entre las dos aponeurosis, en lugar de analizar
          múltiples ROIs pequeñas.
    """
    ...
    # Procesar la imagen
    results, output_mask = process_single_image_171(input_image_path, params, config)
